...
namespace DysonSphereDroneClearing
{
    ...
    public class DysonSphereDroneClearing : BaseUnityPlugin
    {
        ...
        public static void GameMain_Begin_Prefix()
        {
            ...
            {
                //GameMain.data.hidePlayerModel = true;
            ...
            if (___player.factory != null)
            {
                ...
                if (closestVegeDistance <= ___player.mecha.buildArea * configLimitClearingDistance.Value)
                {
                    ...
                    DroneClearingMissionData missionData = new DroneClearingMissionData
                    {
                        ...
                        index = ___player.mecha.idleDroneCount
                    };
                                ...
                                if (configCollectResourcesFlag.Value)
                                {
                                    ...
                                    else
                                    {
                                        if (!missionData.miningFlag)
                                        {
                                            ...
                                            /*prebuild.pos.x += 2;
                                            prebuild.pos2.x += 2;
                                            __instance.position.x += 2;
                                            __instance.target.x += 2;*/
                                            //@@@
                                            Logger.LogMessage("Mining started");
                                            __instance.position = Vector3.zero;
                                            __instance.target = Vector3.zero;
                                            __instance.forward = Vector3.zero;
                                            __instance.initialVector = Vector3.zero;
                                            prebuild.pos = Vector3.zero;
                                            prebuild.pos2 = Vector3.zero;
        ...
        public static bool MechaDrone_Update_Prefix(ref MechaDrone __instance, ref int __result, PrebuildData[] prebuildPool, Vector3 playerPos, float dt, ref double energy, ref double energyChange, double energyRate)
        {
            //@@@
            int prebuildId = -__instance.targetObject;
            if (prebuildId > 0)
            {
                ...
                {
                    __instance.stage = 2;
                    __instance.position = GameMain.mainPlayer.position;

                    for (int activeMissionIdx = 0; activeMissionIdx < activeMissions.Count; ++activeMissionIdx)
                    {
                        DroneClearingMissionData missionData = activeMissions[activeMissionIdx];
                        //Logger.LogWarning($"missionData.prebuildId={missionData.prebuildId}, __instance.targetObject={__instance.targetObject}, missionData.index={missionData.index}");
                        if (missionData.prebuildId == prebuildId)
                        {
                            Vector3 headPosition = GameMain.mainPlayer.position.normalized * (GameMain.mainPlayer.position.magnitude + 2.8f);
                            if (missionData.index == 1)
                            {
                                __instance.position = GameMain.mainPlayer.position.normalized * (GameMain.mainPlayer.position.magnitude + 4.2f);
                                __instance.position.x -= 0.7f;
                                __instance.forward = new Vector3(0, 100, 0);
                            }
                            else if (missionData.index == 2)
                            {
                                __instance.position = GameMain.mainPlayer.position.normalized * (GameMain.mainPlayer.position.magnitude + 4.5f);
                                __instance.position.x += 0.0f;
                            }
                            else if (missionData.index == 3)
                            {
                                __instance.position = GameMain.mainPlayer.position.normalized * (GameMain.mainPlayer.position.magnitude + 4.2f);
                                __instance.position.x += 0.7f;
                                __instance.forward = new Vector3(0, -100, 0);
